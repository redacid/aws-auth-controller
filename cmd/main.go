...
func main() {
	...
	flag.BoolVar(&userNameMustBeEmail, "username-must-be-email", false,
		"Enables checking of username in MapUser, should it be an email address, default false."+
	...
	flag.StringVar(&configMapName, "config-map-name", "aws-auth", "ConfigMap-name where stored items,"+
		" --config-map-name=aws-auth-test")
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "72c9b2c6.prozorro.sale",
	...
	if err := (&controller.MapRoleReconciler{
	...
	if err := (&controller.MapUserReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&controller.MapAccountReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
