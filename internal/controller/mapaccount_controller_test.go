...
var _ = Describe("MapAccount Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind MapAccount")
			err := k8sClient.Get(ctx, typeNamespacedName, mapaccount)
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance MapAccount")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &MapAccountReconciler{
				Client: k8sClient,
