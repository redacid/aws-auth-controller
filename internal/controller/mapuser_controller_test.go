...
var _ = Describe("MapUser Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind MapUser")
			err := k8sClient.Get(ctx, typeNamespacedName, mapuser)
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance MapUser")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &MapUserReconciler{
				Client: k8sClient,
