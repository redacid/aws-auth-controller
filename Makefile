...
IMG ?= redacid/aws-auth-controller-dev:0.0.107
...
CHART_PATH ?= ./charts/aws-auth-operator
...
KIND_CLUSTER ?= aws-auth-controller-prozorro-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name aws-auth-controller-prozorro-builder
	$(CONTAINER_TOOL) buildx use aws-auth-controller-prozorro-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm aws-auth-controller-prozorro-builder
	rm Dockerfile.cross
